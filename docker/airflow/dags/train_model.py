...
def training():
    ...
    def train_model():
        ...
        data = postgres.get_pandas_df(
            f'SELECT * FROM {Variable.get("schema_name")}.{Variable.get("view_training_data")}'
        )
        model = Model(data['metal_type'].unique(), 0, 0)
        model.train(data=data)
        model.save(
            '/tmp/models', s3hook, Variable.get('bucket_name_model_data')
        )
