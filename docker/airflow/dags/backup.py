...
def backup():
    ...
    def __filter_by_interval(objects: list, hours: int):
        ...
        return [
            obj_id for obj_id in objects if __extract_timestamp(obj_id) > interval
        ]
    ...
    def cleanup_backups(subpath: str):
        ...
        backups = sorted(os.listdir(path),
                         key=lambda x: __extract_timestamp(x))
    ...
    def pull_models(models_ids: list):
        ...
        for m_id in models_ids:
            s3hook.download_file(key=m_id,
                                 bucket_name=Variable.get(
                                     "bucket_name_model_data"),
